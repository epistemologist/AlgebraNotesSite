...
- Given two rings $R$ and $S$, the **Cartesian product** of $R \times S$ can be endowed with a ring structure via $$\begin{aligned} + : (R \times S) \times (R \times S ) &\to R \times S \\ (r_1, s_1)  + (r_2, s_2)&\mapsto  (r_1 +_R r_2, s_1 +_S s_2) \end{aligned}  $$ $$ \begin{aligned} \cdot : (R \times S) \times (R \times S ) &\to R \times S \\ (r_1, s_1)  \cdot (r_2, s_2)&\mapsto  (r_1 \cdot_R r_2, s_1 \cdot_S s_2) \end{aligned}$$ (The ring axioms can be verified)
- Given a ring $R$, the set $R[X] = \{ \sum_{i=0}^n a_i x^i : n \ge 0, a_i \in R, a_n \ne 0 \}$ is a ring (**a polynomial ring**) under addition defined by $$ \begin{aligned} + : R[X] \times R[X] &\to R[X] \\ \left( \sum_i a_i X^i \;,\; \sum_{j} b_jX^j \right) &\mapsto \sum_{i} (a_i + b_i) X^i \end{aligned} $$ and multiplication defined by $$\begin{aligned} \cdot: R[X] \times R[X] &\to R[x] \\  \left( \sum_i a_i X^i \;,\; \sum_{j} b_jX^j \right) &\mapsto \sum_k c_k X^k \end{aligned}$$ where $c_k = \sum_{l=0}^k a_l b_{k-l}$.
	- Note that we can view each element $p(X) = \sum_{i=0}^n a_i X^i$ as an infinite series $p(X) = \sum_{k>0} a_k X^k$ with $a_k = 0$ for $k > n$.
	- We have an *embedding* $$ \begin{aligned} R &\hookrightarrow R[X] \\ a &\mapsto a + 0X + 0X^2 + \cdots \end{aligned}$$ similar to $$\begin{aligned} \mathbb{R} &\hookrightarrow \mathbb{C} \\ x &\mapsto x + 0i\end{aligned}$$
...
- Let $G$ be a group, $R$ be a ring. Then a **G-algebra** is a ring on the set $$RG = \left\{ \sum_{g \in G} a_g g: g \in G: a_g \in R \right\}$$ with addition defined via $$\left( \sum_{g \in G} a_g g \right) + \left( \sum_{g \in G} b_g g\right) = \left(\sum_{g \in G} (a_g + b_g) g \right)$$ and multiplication defined via $$\left( \sum_{g \in G} a_g g\right) \cdot \left( \sum_{g \in G} b_g g\right) = \sum_{g \in G} c_g g$$ where $c_g = \sum_{\substack{ h,k \in G \\ hk=g} } a_h b_k$
	...
	- Examples
		- $R = \mathbb{Z}, G = \mathbb{Z}_2 \cong \{ x : x^2 = 1\}$ - then $RG = \{ a+bx : x^2 = 1\}$, $$ \begin{aligned} (3+2x) \cdot (5+3x) &= 15 + 9x - 10x - 6x^2 \\ &= 9 - x\end{aligned}$$
	...
	- Note if $g \in G$ has finite order, then $$\begin{aligned} 0 &= 1-g^n \\ &= \underbrace{ (1-g) }_{\ne g} \underbrace{ (1 + g + g^2 + \cdots + g^{n-1})}_{\ne g}\end{aligned}$$ Therefore, any group algebra where $G$ has an element of finite order is *not* an integral domain
...
-  An **algebra** over a field $F$ is a ring $R$ that contains of copy of $F$ in its center $Z(R)$
	- For example, we have $$\begin{aligned} F &\hookrightarrow F[x] \\ a &\mapsto a+0x+0x^2+ \cdots\end{aligned}$$ Since $a \in F$ commutes with all elements of $F[x]$, we have $F \subseteq Z(F[x])$ and therefore $F[x]$ is the *algebra of polynomials over $F$.*
	- Similarly, we have $$\begin{aligned} F &\hookrightarrow M_n(F) \\ a &\mapsto a \cdot I_n\end{aligned}$$and so $M_n(F)$ is the *algebra of matrices over $F$*
	- We also have the embedding $$\begin{aligned}F \hookrightarrow FG  \\ a \mapsto a\cdot 1_G\end{aligned}$$and so $FG$ is the *group algebra of $G$ over $F$*.
...
- Examples of the above construction
	...
	- The **Weyl algebra**: A $F$-vector space $F\langle x,y \rangle$ where basis elements multiply like $y x = x y + 1$. For example, we have: $$ \begin{aligned} y x^2 y &= y x x y \\ &= (xy+1) xy \\ &= xyxy + xy \\ &= x(xy+1)y + xy \\ &=  (x^2 y + x)y + xy \\ &= x^2y^2 + 2xy  \end{aligned}$$
	- The **quantum plane**: $\mathbb{C}_q[x,y] = \text{span} \{ x^i y^j : i,j \ge 0\}$ subject to $yx = qxy$ where $q \in \mathbb{C} \setminus \{0,1 \}$; for example, we have $$\begin{aligned} yx^2y &= y x x y \\ &= q xyxy \\ &= qx(qxy)y \\ &=q^2 x^2 y^2  \end{aligned}$$
	...
	- $R = \text{span}_F \left\{ x^i y^j : i,j \ge 0\right\}$ subject to relations $yx = xy - x$; for example, we have: $$\begin{aligned} y^2x = yyx &= y(xy+x^2) \\ &= yxy + yx \\ &=(xy+x)y + (xy+x) \\ &= xy^2 + xy + xy+ x = xy^2 + 2xy+x \end{aligned}$$
...
- examples of Lie algebras
	- the *trivial/abelian Lie algebra*: for any vector space $L$ over field $F$, define $[x,y]=0 \; \forall x,y = 0$
		- every one-dimensional Lie algebra is abelian
	- 2 dimensional examples - let $L = \text{span}\{x,y\}$
		- trivial: we have:  $$ \begin{aligned} [x,x] = [y,y] = 0 \\ [x,y] = 0 \implies [y,x] = 0\end{aligned}$$
		- *non-trivial*: $[x,y] = ax+by \implies [x,y]=x$ through change of basis
	- any associative algebra $A$ over a field $F$ is a lie algebra as we have $[x,y] = xy-yx$; e.g. we have $M_n(F)$ is a Lie algebra of $GL_n(F)$ 
