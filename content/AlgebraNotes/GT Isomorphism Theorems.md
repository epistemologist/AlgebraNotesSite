...
# GT Isomorphism Theorems
 - given groups $H \le G, g \in G$ we can define **left/right cosets** $$\begin{aligned} gH &= \{ gh: h \in H \} \\ Hg &= \{hg: h \in H \} \end{aligned}$$
 ...
 - let $G$ be a group with $N \trianglelefteq G$ - we can define a product operation on $G/N$ via $$ \begin{aligned} \cdot : G/N \times G/N &\to G/N \\ (xN) \cdot (yN) &= (xy)N \end{aligned}$$ this endows $G/N$ with a group structure - we call this the **quotient group** of $G$ by $N$ 
 - let $N \trianglelefteq G$ - then the function $$\begin{aligned} \varphi: G &\to G/N \\ x &\mapsto xN \end{aligned}$$ is a group homomorphism (the *canonical quotient homomorphism*) with $\ker\varphi = N$. 
 ...
 - isomorphism theorems
	 ...
	 - ***First Isomorphism Theorem***: Let $\varphi: G \to H$ be a group homomorphism. Then the function $$\begin{aligned} \phi: G / \ker(\varphi) &\to \text{im}(\varphi)  \\ g \cdot \ker(\varphi) &\mapsto \varphi(g) \end{aligned}$$ is a well-defined group isomorphism. We therefore have that $G/\ker(\varphi) \cong \text{im}(\varphi)$ 
		 - we can use this theorem to show two groups are isomorphic - for example, we can show that if $G = \langle x \rangle$ with $|G| = n$, then $G \cong \mathbb{Z}/n\mathbb{Z}$; we show this by defining $$\begin{aligned}\varphi: \mathbb{Z} &\to G \\ m &\mapsto x^m \end{aligned}$$ and then showing that $\varphi$ is a surjective homomorphism with $\ker\varphi = n\mathbb{Z}$  
	 ...
	 - **Lattice Isomorphism Theorem**: Let $N \trianglelefteq G$ and let $\varphi: G \to G/N$ be the canonical quotient homomorphism. Then the functions $$ \begin{aligned} \Phi: \{  A: A \le G , N \le A \} &\to \{B : B \le G /N\} \\ A &\mapsto A/N \end{aligned} $$ and $$ \begin{aligned} \Psi: \{ B: B \le G/N \} &\to \{ A : A \le G, N \le A \} \\ B &\mapsto \varphi^{-1}(B) \end{aligned} $$ are inclusion-preserving inverse bijections. Moreover, given $A_1, A_2$ in the domain of $\Psi$, we have $$\begin{aligned} A_1 \le A_2 & \iff \Phi(A_1) \le \Phi(A_2) \\ A_1 \le A_2 &\implies [A_2 : A_1] = [\Phi(A_2) : \Phi(A_1)]  \\ \Phi(\langle A_1, A_2 \rangle)&= \langle \Phi(A_1), \Phi(A_2) \rangle \\ \Phi(A_1 \cap A_2 ) &=  \Phi(A_1) \cap\Phi(A_2) \\ A_1 \trianglelefteq G&\iff \Phi(A_1) \trianglelefteq G/N  \end{aligned}$$
 - composition series / Holder program
	 ...
	 - for a group $G$, a **normal series** for $G$ is a finite sequence $\{e\} = N_0, N_1,  \cdots, N_{k-1}, N_k = G$ of subgroups of $G$ such that $$ \{e\} = N_0 \trianglelefteq N_1 \trianglelefteq \cdots \trianglelefteq N_k \trianglelefteq N_k = G$$ The corresponding quotient groups $N_{i+1}/N_i$ are called the *quotients*/*factors* of the normal series. A **composition series** is a normal series where all quotients are simple groups
		 - composition series are not necessary unique - for example, for $G = \mathbb{Z}_3 \times \mathbb{Z}_5$, we have $$ \begin{aligned}\{ e \} \trianglelefteq \mathbb{Z}_3 \times \{ e \} \trianglelefteq \mathbb{Z}_3 \times \mathbb{Z}_5 \\ \{ e \} \trianglelefteq \{ e \} \times \mathbb{Z}_5 \trianglelefteq \mathbb{Z}_3 \times \mathbb{Z}_5  \end{aligned} $$
